...
For GPU access you will need to create a `/etc/docker/daemon.json` with the
following content where `GPU-f0ada461` come from the command line below
...
```json
{
  ...
  "node-generic-resources": ["NVIDIA-GPU=GPU-f0ada461"]
}
```
